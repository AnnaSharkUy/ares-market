...
Уникальность AresMarket — марсианская тематика, валюта Sols, колонии, богатый e-commerce флоу.
...
Данные (SQLite + uploads) хранятся в volume `ares-data`.

## Роли

| Роль | Возможности |
|------|-------------|
| **guest** (без входа) | Каталог, карточки, локаль, guide, обратная связь, просмотр баг-репортов |
| **colonist** | + корзина, заказы, отзывы |
| **admin** | как colonist (расширенная админка вне scope) |

Корзина и заказы требуют JWT. Гость при попытке оформить заказ будет направлен на вход.

## Обратная связь

Меню → **Обратная связь**: форма с вложениями (PDF, изображения, Office…). Есть учебные ловушки в валидации размера/типа/числа файлов.

## Локаль

- `ru` / `en` — OK  
- `mars` — HTTP **200**, но строки усечены / на неправильном языке (тонкие баги UI)  
- `GET /api/locale/mars/crash` — по-прежнему **500** (для API-исследователей)
...
Сделано департаментом тестирования компании Рексофт для программы стажировки по функциональному / full-stack тестированию в 2026году.  
